...
import { LayoutDashboard, Calendar, Tags, Users, UserCheck, Settings, LogOut, Menu, X, Zap } from 'lucide-react';
    ...
    return (
        <div data-testid="admin-dashboard">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
                {cards.map((c, i) => (
                    <div key={i} className="glass-card rounded-xl p-5 hover:border-white/10 transition-colors" data-testid={`stat-${c.label.toLowerCase().replace(/\s/g,'-')}`}>
            ...
            {stats.active_event && (
                <div className="glass-card rounded-xl p-6">
                    ...
                    <p className="text-xl font-bold text-white">{stats.active_event.name}</p>
...
export default function AdminLayout() {
    ...
    return (
        <div className="min-h-screen bg-background flex" data-testid="admin-layout">
            ...
            {/* Sidebar */}
            <aside className={`fixed lg:sticky top-0 left-0 z-50 h-screen w-64 bg-[#0A0A0A] border-r border-white/5 flex flex-col transition-transform duration-200 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
                <div className="p-6 flex items-center gap-3 border-b border-white/5">
                    <div className="w-9 h-9 rounded-lg bg-primary/20 flex items-center justify-center">
                        <Zap className="w-5 h-5 text-primary" />
                    </div>
                    <span className="text-xl font-black tracking-tighter" style={{fontFamily:'Outfit'}}>SSNC</span>
                    <span className="text-xs text-muted-foreground ml-auto bg-[#171717] px-2 py-0.5 rounded">Admin</span>
                </div>
                    ...
                    {navItems.map(item => {
                        ...
                        return (
                            <Link key={item.path} to={item.path} onClick={() => setSidebarOpen(false)}
                                className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors ${active ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:text-white hover:bg-white/5'}`}
                ...
                <div className="p-4 border-t border-white/5">
                    <Button variant="ghost" className="w-full justify-start gap-3 text-muted-foreground hover:text-white" onClick={handleLogout} data-testid="admin-logout-btn">
            ...
            <main className="flex-1 min-h-screen">
                <header className="sticky top-0 z-30 bg-background/80 backdrop-blur-xl border-b border-white/5 px-6 py-4 flex items-center gap-4 lg:hidden">
                    <Button variant="ghost" size="icon" onClick={() => setSidebarOpen(!sidebarOpen)} data-testid="mobile-menu-btn">
                        {sidebarOpen ? <X size={20} /> : <Menu size={20} />}
                    </Button>
                    <span className="text-lg font-bold" style={{fontFamily:'Outfit'}}>SSNC Admin</span>
                </header>
